      ...
      { selectedPokemon && !isLoading &&
        ...
        bg-type-${selectedPokemon.types[0].type.name}`}>
            <div className="flex w-full justify-around">
                ...
                <div className="w-[100px] h-[100px] rounded-3xl border-2 
            ...
            <div className="flex gap-4 pr-4">
                    {selectedPokemon.types.map((type, index) => (
                        <div key={index} className="w-fit h-[32px] bg-background flex items-center rounded-3xl gap-2 px-2">
                            <h3 className="text-[8px]">{type.type.name.toUpperCase()}</h3>
                            <img className="w-[24px]" src={getTypeIcon(type.type.name)} alt=""/>
                        </div>
                    ))}
                </div>
