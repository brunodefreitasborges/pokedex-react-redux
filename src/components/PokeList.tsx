             ...
             <div className="flex justify-between px-4">
              {
                page > 1 && page < 11 &&
                <>
                  <p data-testid="previous-button" onClick={() => {dispatch(fetchPokemonList(getOffsetFromUrl(pokemonPage!.previous), false))}} className="px-2 py-1 hover:cursor-pointer hover:bg-white hover:text-black">Previous</p>  
                  <p data-testid="next-button" onClick={() => {dispatch(fetchPokemonList(getOffsetFromUrl(pokemonPage!.next), true))}} className="px-2 py-1 hover:cursor-pointer hover:bg-white hover:text-black">Next</p>
                </>
              }
              {
                page == 1 &&
                <>
                  <p data-testid="previous-button" className="px-2 py-1 text-gray-500">Previous</p>  
                  <p data-testid="next-button" onClick={() => {dispatch(fetchPokemonList(getOffsetFromUrl(pokemonPage!.next), true))}} className="px-2 py-1 hover:cursor-pointer hover:bg-white hover:text-black">Next</p>
                </>
              }
              {
                ...
                <>
                  <p data-testid="previous-button" onClick={() => {dispatch(fetchPokemonList(getOffsetFromUrl(pokemonPage!.previous), false))}} className="px-2 py-1 hover:cursor-pointer hover:bg-white hover:text-black">Previous</p>  
                  <p data-testid="next-button" className="px-2 py-1 text-gray-500">Next</p>
                </>
              }
